    ...
    public void afterConnectByFrom(AbstractFlowNodeBuilder<?, ?> cmdNode,
        ...
        if(childNode!=null) {
            ...
            if(StringUtils.isNotBlank(parentId)){
                ...
                if(parentNode!=null){
                    if(CamundaNodeType.CONDITION.isEqual(parentNode.getType()) || CamundaNodeType.INCLUSIVE.isEqual(parentNode.getType()) ){
...
//                                                String str = StringUtils.join(value, ",");
                                        ...
                                        else if("Role".equals(valueType)){
                                            List<String> roleIds=new ArrayList<>();
                                            for (Object o : value) {
                                                JSONObject obj=(JSONObject)o;
                                                roleIds.add(obj.getString("key"));
                                            }
                                            String str=" "+ CamundaWorkConstants.EXPRESSION_CLASS+"roleStrContainsMethod(\"{0}\",\"{1}\",    execution) ";
                                            str = str.replace("{0}", id);
                                            str = str.replace("{1}", StringUtils.join(roleIds, ","));
                                            conditionExpression.append(str );
                                        }
                                        else{
                                            continue;
