...
public class SettleConverter  implements Converter<Integer>  {
    ...
    public Integer convertToJavaData(CellData cellData, ExcelContentProperty excelContentProperty, GlobalConfiguration globalConfiguration) throws Exception {
        ...
        if ("外部结算".equals(stringValue)) {
            return 0;
        }else if ("内部结算".equals(stringValue)){
            return 1;
        }
    ...
    public CellData convertToExcelData(Integer s, ExcelContentProperty excelContentProperty, GlobalConfiguration globalConfiguration) throws Exception {
        if (s.equals(0)) {
            return new CellData<>("外部结算");
        }else if (s.equals(1)){
            return new CellData<>("内部结算");
        }
